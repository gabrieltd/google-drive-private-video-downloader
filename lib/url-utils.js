...
export function extractDriveFolderNameFromTitle(title) {
    ...
    const name = title
        ...
        .trim();
    if (!name || /^(?:Google Drive|My Drive)$/i.test(name)) return null;
    return name;
}

export function resolveDriveFolderName(primaryName, fallbackName) {
    for (const value of [primaryName, fallbackName]) {
        if (typeof value !== "string") continue;
        const name = value.replace(/\s+/g, " ").trim();
        if (name && !/^(?:Google Drive|My Drive)$/i.test(name)) return name;
    }
