...
export function enrichParsedVideoSource(parsedVideo, playbackFileId) {
    const fileId = typeof playbackFileId === "string" ? playbackFileId.trim() : "";
    if (!parsedVideo || typeof parsedVideo !== "object" || !fileId) return parsedVideo;

    const sourceMetadata = { ...(parsedVideo.sourceMetadata ?? {}) };
    if (sourceMetadata.fileId) return parsedVideo;

    sourceMetadata.fileId = fileId;
    if (!sourceMetadata.stableSourceKey || sourceMetadata.stableSourceKey.startsWith("fallback:")) {
        sourceMetadata.stableSourceKey = `source:${fileId}`;
    }

    return { ...parsedVideo, sourceMetadata };
}
