...
import {
    ...
    findFolderDownloadByDownloadId,
...
export function createTabStateManager({ storage, storageKey = DEFAULT_STORAGE_KEY } = {}) {
    ...
    return {
        ...
        updateFolderDownloadItem(tabId, itemKey, changes) {
            const state = states.get(tabId);
            if (!state) return null;
            state.folderScan.downloadItems = updateFolderDownloadItem(
                state.folderScan.downloadItems,
                itemKey,
                changes,
            );
            return clone(state.folderScan.downloadItems.find((item) => item.key === itemKey) ?? null);
        },

        findFolderDownloadByDownloadId(downloadId) {
            for (const state of states.values()) {
                const item = findFolderDownloadByDownloadId(state.folderScan.downloadItems, downloadId);
                if (item) return { tabId: state.tabId, scanId: state.folderScan.id, item: clone(item) };
            }
            return null;
        },
